...
import static com.mihapetr.socialnetwork.domain.CommentAsserts.*;
...
import com.mihapetr.socialnetwork.repository.CommentRepository;
...
class CommentResourceIT {
    ...
    public static Comment createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, Message.class).isEmpty()) {
            message = MessageResourceIT.createEntity();
            em.persist(message);
            em.flush();
        } else {
    ...
    public static Comment createUpdatedEntity(EntityManager em) {
        ...
        Message message;
        if (TestUtil.findAll(em, Message.class).isEmpty()) {
            message = MessageResourceIT.createUpdatedEntity();
            em.persist(message);
            em.flush();
        } else {
            message = TestUtil.findAll(em, Message.class).get(0);
        }
        updatedComment.setParent(message);
        // Add required entity
        Post post;
        if (TestUtil.findAll(em, Post.class).isEmpty()) {
