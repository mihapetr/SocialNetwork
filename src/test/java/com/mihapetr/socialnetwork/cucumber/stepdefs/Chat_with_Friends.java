...
public class Chat_with_Friends extends Common {
    ...
    void createChatRequestForUser() {
        ...
        Chat chat = new Chat().initiatorName("seconduser").accepted(false).addChat(
            requestMessage
        ).addProfile(mainProfile).addProfile(requestProfile);
        requestChatId = chatRepository.save(chat).getId();
    }
    void createFriendForChat() {
        Message requestMessage = new Message().content("I want to chat!");
        requestMessage = messageRepository.save(requestMessage);
        Chat chat = new Chat().initiatorName("thirduser").accepted(true).addChat(
            requestMessage
        ).addProfile(mainProfile).addProfile(chatProfile);
	...
	public void user_navigates_to_Profiles_view() {
        profilesResponse = restClient.get()
            .uri("http://localhost:" + port + "/api/profiles")
            .header("Authorization", "Bearer " + token)
            .retrieve().toEntity(new ParameterizedTypeReference<List<Profile>>() {}).getBody();
        assert profilesResponse != null && profilesResponse.size() == 4 :
            "Profiles response is null or not of predicted size; size = " + profilesResponse.size() ;
	}
	...
	public void user_selects_Profile_details() {
        selectedProfileId = profilesResponse.stream().filter(profile -> profile.getUser().getLogin().equals("firstuser"))
	...
	public void user_is_not_other_profile_s_friend_already() {
        Profile selectedProfile = restClient.get().uri("http://localhost:" + port + "/api/profiles/" + selectedProfileId)
            .header("Authorization", "Bearer " + token)
            .retrieve().toEntity(Profile.class).getBody();
        Profile userProfile = restClient.get().uri("http://localhost:" + port + "/api/profiles/current-user")
            .header("Authorization", "Bearer " + token)
            .retrieve().toEntity(Profile.class).getBody();
	...
	public void chat_with_request_is_created_with_the_other_profile() {
        ...
        assert dbChat.getInitiatorName().equals(existingLogin)
            : "user who requested chat/friends unexpected; initiatorName = " + dbChat.getInitiatorName();
	...
	public void user_navigates_to_Chats_view() {
        userChats = restClient.get().uri("http://localhost:" + port + "/api/chats")
            .header("Authorization", "Bearer " + token)
            .retrieve().toEntity(new ParameterizedTypeReference<List<Chat>>() {
            }).getBody();
	}

	@And("user has a friend request message from other profile")
	public void user_has_a_friend_request_message_from_other_profile() {
        assert userChats.stream().anyMatch(chat -> chat.getInitiatorName().equals("seconduser")) :
	...
	public void user_opens_the_chat_with_request() {
        chatWithRequest = restClient.get().uri("http://localhost:" + port + "/api/chats/" + requestChatId)
            .header("Authorization", "Bearer " + token)
            .retrieve().toEntity(Chat.class).getBody();
	}
	...
	public void user_selects_accept_option() {
        updatedProfile = restClient.patch().uri("http://localhost:" + port + "/api/chats/" + requestChatId + "/accept")
            .header("Authorization", "Bearer " + token)
            .retrieve().toEntity(Profile.class).getBody();
	}
	...
	public void user_and_other_profile_are_friends() {
        ...
        assert inOthers || inProfiles :
            "No initiator in user's others nor profiles; initiatorLogin = " + initiatorLogin  + ", others = " + others + ", profiles = " + profiles;
	}
	...
	public void user_opens_a_Chat_with_a_recipient() {
        chatWithOther = restClient.get().uri("http://localhost:" + port + "/api/chats/" + friendChatId)
            .header("Authorization", "Bearer " + token)
            .retrieve().toEntity(Chat.class).getBody();
	}
	...
	public void user_is_recipient_s_friend() {
        ...
        assert others.stream().anyMatch(profile -> profile.getUser().getLogin().equals(chatFriendLogin))
            || profiles.stream().anyMatch(profile -> profile.getUser().getLogin().equals(chatFriendLogin)) :
            "user is not recipient's friend; others = " + userProfile.getOthers() + "; profiles = " + userProfile.getProfiles();
	}
	...
	public void user_submits_Message_content() {
        ...
        newMessage = restClient.patch().uri("http://localhost:" + port + "/api/chats/" + chatWithOther.getId() + "/message")
            .header("Authorization", "Bearer " + token)
            .body(newMessage)
            .retrieve().toEntity(Message.class).getBody();
	}

	@Then("message is created in the chat")
	public void message_is_created_in_the_chat() {
        chatUserView = restClient.get().uri("http://localhost:" + port + "/api/chats/" + chatWithOther.getId())
            .header("Authorization", "Bearer " + token)
            .retrieve().toEntity(Chat.class).getBody();

        token2 = signIn("seconduser", "seconduserpass");
        chatFriendView = restClient.get().uri("http://localhost:" + port + "/api/chats/" + chatWithOther.getId())
	...
	public void both_users_can_see_the_message() {
        assert chatUserView != null &&
            chatUserView.getChats().stream().anyMatch(chat -> chat.getContent().equals(content)) :
